-- scorpion ui lib reuploaded by me
-- init
...
do
	...
	function section:addText(realname, title, selectedvalue,callback)
		...
		local text = textlabel[realname]
		
		return textlabel
	end

	function section:updateTextV2(info, realname, title, selectedvalue)
		info = self:getTextModule(info,realname)
	
		info[realname].Text = title,selectedvalue
	end
	
	function section:updateText(textlabel, title, selectedvalue)
	...
	function section:addSliderS(title, default, min, max, callback)
		local slider = utility:Create("ImageButton", {
			Name = "Slider",
			Parent = self.container,
			BackgroundTransparency = 1,
	...
	function section:getTextModule(info,realname)
		...
		for i, module in pairs(self.modules) do
		print(info,realname,":",module:FindFirstChild(info)[realname])
			if module:FindFirstChild(info)[realname].Name == realname then
				return module
			end
		end

		error("No Text module found under "..tostring(info,realname))
	end
